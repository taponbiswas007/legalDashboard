...
<x-app-layout>
                ...
                <div class="welcome-card bg-gradient-primary rounded-3 p-4 text-white shadow">
                    <div class="row align-items-center">
                        ...
                        <div class="col-md-4 text-md-end">
                            <div
                                class="stats-badge bg-white text-black bg-opacity-20 rounded-pill px-3 py-2 d-inline-block">
                    ...
                    <div class="card-body">
                        <div class="action-grid">
                            <a href="{{ route('addcase.create') }}"
                                class="action-card bg-primary text-white rounded-3 p-3 text-center text-decoration-none">
                                <i class="fas fa-plus fa-2x mb-2"></i>
                                <h6 class="mb-0">New Case File</h6>
                            </a>
                            <a href="{{ route('addclient.create') }}"
                                class="action-card bg-info text-white rounded-3 p-3 text-center text-decoration-none">
                                <i class="fas fa-user-plus fa-2x mb-2"></i>
                                <h6 class="mb-0">Add Client</h6>
                            </a>
                            <a href="{{ route('notes.create') }}"
                                class="action-card bg-success text-white rounded-3 p-3 text-center text-decoration-none">
                                <i class="fas fa-sticky-note fa-2x mb-2"></i>
                                <h6 class="mb-0">Add Note</h6>
                            </a>
                            <a href="{{ route('dashboard.daily_work') }}"
                                class="action-card bg-warning text-white rounded-3 p-3 text-center text-decoration-none">
                                ...
                                <div class="d-none d-md-block">
                                    <div class="row align-items-center">
                                        ...
                                        <div class="col-md-2 text-center">
                                            @if ($case->next_hearing_date)
                                                <div
                                                    class="time-slot bg-warning bg-opacity-10 rounded-2 p-2 d-inline-block">
                                                    <div class="h6 mb-0 text-warning">
                                                        {{ \Carbon\Carbon::parse($case->next_hearing_date)->format('h:i A') }}
                                                    </div>
                                                    <small class="text-muted">
                                        ...
                                        <div class="col-md-2 text-end">
                                            <a href="{{ route('addcase.show', Crypt::encrypt($case->id)) }}"
                        ...
                        <div class="notes-feed">
                            @forelse($recentNotes as $note)
                                <div class="note-item mb-3 p-3 border rounded-2">
                                    <div class="d-flex justify-content-between align-items-start mb-2">
                                        <h6 class="mb-0 text-dark fw-bold fs-6">{{ Str::limit($note->title, 35) }}
                                        </h6>
                                    ...
                                    <p class="text-muted small mb-2">
                                        {{ Str::limit(strip_tags($note->description), 70) }}</p>
                                    <div class="d-flex justify-content-between align-items-center">
                                        <small class="text-muted">
                                            <i class="fas fa-clock me-1"></i>
                    ...
                    <div class="modal-body">
                        <div class="row g-3">
                            <div class="col-12">
                                ...
                                <input type="text" name="title" class="form-control rounded" id="edit_title"
                                    required>
                            </div>
    ...
    <!-- Update Case Modal -->
    <div class="modal fade" id="updateCaseModal" tabindex="-1" aria-labelledby="updateCaseModalLabel"
        aria-hidden="true">
                ...
                <form id="updateCaseForm">
                    ...
                    <div class="modal-body p-4">
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control border-0 bg-light" id="modalFileNumber"
                                readonly>
                        </div>

                        <div class="mb-3">
                            <label for="next_hearing_date" class="form-label fw-bold text-dark">Next Hearing Date
                                <span class="text-danger">*</span></label>
                            <input type="date" class="form-control border-0 bg-light" id="next_hearing_date"
                                name="next_hearing_date" required>
                            <div class="form-text text-muted">Select the next court hearing date</div>
                        </div>

                        <div class="mb-3">
                            <label for="next_step" class="form-label fw-bold text-dark">Next Step <span
                                    class="text-danger">*</span></label>
                            <textarea class="form-control border-0 bg-light" id="next_step" name="next_step" rows="3"
                                placeholder="Enter the next legal step or action required..." required></textarea>
                        ...
                        .then(data => {
                            if (data.success) {
                                ...
                                const badge = document.querySelector(
                                    `.status-badge[data-id="${currentNoteId}"]`);

                                // Remove existing status classes
                                badge.classList.remove('pending-status', 'done-status',
                                    'reject-status');
                                // Add new status class
                                badge.classList.add(`${newStatus.toLowerCase()}-status`);

                                // Update icon and text
                                const iconClass = newStatus === 'Pending' ?
                                    'fa-solid fa-clock' :
                                    newStatus === 'Done' ? 'fa-solid fa-check' :
                                    'fa-solid fa-xmark';
