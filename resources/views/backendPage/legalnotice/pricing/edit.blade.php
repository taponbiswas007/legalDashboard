        ...
        <div class="card shadow rounded border-0">
            ...
            <div class="card-body">
                ...
                <div class="alert alert-info alert-dismissible fade show" role="alert">
                    ...
                    <strong>Current Pricing:</strong> {{ $pricing->client?->name }} - {{ $pricing->category?->name }} =
                    ৳ {{ number_format($pricing->price, 2) }}
                    <button type="button" class="btn-close" data-bs-dismiss="alert"></button>
                </div>

                <form action="{{ route('legalnotice.pricing.update', $pricing->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                            ...
                            <div class="position-relative">
                                <select name="client_id" class="form-select @error('client_id') is-invalid @enderror"
                                    ...
                                    @foreach ($clients as $client)
                                        <option value="{{ $client->id }}"
                                            {{ old('client_id', $pricing->client_id) == $client->id ? 'selected' : '' }}>
                                            {{ $client->name }}
                                        </option>
                                    @endforeach
                                </select>
                                ...
                                <div class="dropdown">
                                    <input type="text"
                                        ...
                                        value="{{ old('client_id', $pricing->client_id) ? $clients->where('id', old('client_id', $pricing->client_id))->first()->name ?? '' : '' }}">
                            ...
                            <div class="position-relative">
                                <select name="category_id"
                                    ...
                                    @foreach ($categories as $category)
                                        <option value="{{ $category->id }}"
                                            {{ old('category_id', $pricing->category_id) == $category->id ? 'selected' : '' }}>
                                            {{ $category->name }}
                                        </option>
                                    @endforeach
                                </select>
                                ...
                                <div class="dropdown">
                                    <input type="text"
                                        ...
                                        data-bs-toggle="dropdown" autocomplete="off"
                                        value="{{ old('category_id', $pricing->category_id) ? $categories->where('id', old('category_id', $pricing->category_id))->first()->name ?? '' : '' }}">
